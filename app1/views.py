...
def jobseekersignup(request):
     if request.method=='POST':
        ...
        user=User.objects.create_user(first_name=f,last_name=l,username=e,password=p)
        jobskrsignup.objects.create(user=user,mobile=m,gender=g,education=ed,type="jobseeker")
...
def employerhome(request):
    return render(request,'empr/emprhome.html')  

def myprofileempr(request):
    if not request.user.is_authenticated:
        return redirect('emprlogin')
    job = emprsignup.objects.get(user=request.user)
    d={'job':job}
    return render(request,'empr/myproempr.html',d)
...
def addjob(request):
    ...
    if request.method=='POST':
        ...
        try:
            Job.objects.create(recruiter=recruiter,startdate=s,enddate=e,title=j,salary=sa,description=de,experiance=ex, location=l,skills=sk,workmode=wm,creationdate=date.today())
            error="no"
        except:
            error="yes"
    d = {'error':error}
...
def jobseekerhome(request):
    return render(request,'jk/jobskrhome.html')

def myprofilejobskr(request):
    if not request.user.is_authenticated:
        return redirect('jobskrlogin')
    
    job = jobskrsignup.objects.get(user=request.user)
    d={'job':job}
    return render(request,'jk/myprojobskr.html',d)
